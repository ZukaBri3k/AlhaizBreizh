    ...
    public function getInfoLogement(Request $request) {
        if(!auth()->check()) {
            ...
            if(auth()->user()->role == 1) {
                ...
                if($id_resa != null){
                    foreach($values as $id_resa) {
                        $resa = DB::select('select id_reserv_avis from avis inner join reservation on avis.id_reserv_avis = reservation.id_reserv inner join logement on avis.id_logement_avis = logement.id_logement where id_reserv = ? and id_logement_avis = ?', [$values, intval($request->id)]);
                        if($resa != null) {
                            $bool_resa = true;
                        } else {
                            $bool_resa = false;
