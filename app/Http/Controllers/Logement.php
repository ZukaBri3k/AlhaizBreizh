...
class Logement extends Controller
{
    ...
    public function getInfoLogement(Request $request) {
        ...
        'avis' => DB::select('select pseudo_pers, ville_pers, pays_pers, photo_pers, com_avis, note_avis from personnes inner join avis on personnes.id = avis.id_personne_avis where id_logement_avis = ?', [intval($request->id)]),
    ]);
    }
